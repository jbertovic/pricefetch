...
use actors::{
    DataStoreBuffer, DataWriterCsv, DataWriterStdout, QuoteRequest, QuoteRouter, StockDataProcessor,
};
use async_std::{
    stream, fs::File, io::BufWriter, prelude::*, task
};
...
pub fn run_program(
    ...
    task::block_on(async {
        ...
        let _file_writer = match file_name {
            Some(name) => {
